...
let isExploding = false; // Flag to track explosion state
let explosionRadius = 0; // Radius of explosion
let explosionX, explosionY; // Position of the explosion
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (distance < explosionRadius) {
          ...
          // Calculate the displacement amount based on distance
          let displacement = map(distance, 0, explosionRadius, 0, 10);
          ...
          let originalColor = img.get(newX, newY);
...
function mouseDragged() {
  // Increase the explosion radius while the mouse is dragged
  let dx = mouseX - explosionX;
  let dy = mouseY - explosionY;
  explosionRadius = sqrt(dx * dx + dy * dy);
}
