...
function setup() {
	...
	xScale = width/100;
	yScale = height/100*(width/height);
	
	centerX = width/2;
	centerY = height/2;
	
	colors = [color("#DDA545"), color("#DBFDDA"), color("#6D6356"), color("#FFDCCB"), color("#CCBEA2"), color("#C0A4AC"), color("#815B47")];
}
...
function draw() {
	...
	*/
	...
	if(length == 0){
		...
		textSize(40);
        textFont("Unbounded");
		text("Paint like God", centerX, centerY);
		return;
	}
...
function getSlopeY(x, y){
	switch(variation){
		...
		case 2:return Math.cos(random(x*y));
		case 3:return Math.sin(x)*Math.tan(y);
		case 4:return Math.cos(x)*y*y;
		case 5:return Math.log(Math.abs(x/10))*Math.log(Math.abs(y/10));
		case 6:return Math.tan(x)*Math.cos(y);
		case 7:return -Math.sin(x*0.1)*3;//orbit
		case 8:return (x-x*x*x)*0.3;//two orbits
...
function getSlopeX(x,y){
	switch(variation){
		...
		case 1:return Math.sin(y*2)*x/y*0.3;
		case 2: return 10;
		case 3: 
		case 4: 
		case 5: 
		case 6:return 1;
		case 7:return Math.sin(y*0.1)*0.5;//orbit
		case 8:return y/0.5;//two orbits
		case 9:return -y;		
		case 10:return -3.5*y;
		case 11:return Math.sin(x)*Math.cos(x);
	}
}
